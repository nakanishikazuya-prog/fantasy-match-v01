...
function buildArticlePrompt(input, classRule) {
  ...
  return [
    ...
    '- 決着は次のいずれかにする。(1)KO/TKOによる決着。(2)判定に持ち込まれた場合は、ジャッジ3者それぞれが「選手Aを支持」「選手Bを支持」「五分(イーブン)」のいずれかを採点したと考え、その組み合わせだけから機械的に結果を決めること。以下の組み合わせと結果の対応表に完全に従い、これ以外の呼称・組み合わせは絶対に使わない。(a)3名とも同じ選手を支持→その選手の「3-0(ユナニマスデシジョン)」勝利。(b)2名が同じ選手を支持し、残り1名がもう一方の選手を支持→2名に支持された選手の「2-1(スプリットデシジョン)」勝利。(c)2名が同じ選手を支持し、残り1名が五分→2名に支持された選手の「2-0(マジョリティデシジョン)」勝利。(d)1名が選手Aを支持、1名が選手Bを支持、残り1名が五分→どちらの選手も2名の支持を得ていないため「引き分け(マジョリティドロー)」とし、絶対にどちらかの勝利にしない。(e)3名とも五分、または2名以上が五分→「引き分け(ドロー)」とする。重要: 「2-0のマジョリティデシジョン」を使ってよいのは、2名が明確に同じ選手を支持している場合のみ。判定が選手Aと選手Bに割れている場合(上記(d))は、必ず引き分けにすること。実力差が明確な組み合わせでは(a)の3-0を普通に使ってよい。内容(実力差・体格差・ラウンド数)に見合ったスコアにし、僅差にするために無理に結果を歪めない。(3)両者の相性(接近戦志向が強い、頭を低くして突っ込むスタイル同士など)によっては、偶発的なバッティング(頭突き)による負傷が原因で続行不可能になり、規定ラウンドまでの採点で決する「負傷判定(テクニカルデシジョン)」を選んでもよい。これは頻発させず、両者のスタイルから偶発的な接触が起こりやすいと言える場合にのみ使う',
